//! Test how peak gas changes with wind=0 (matching user's configuration)
...
fn main() {
    ...
    println!("Peak Shaver Mode - Storage vs Peak Gas (Rust, Wind=0):");
    println!("{}", "=".repeat(70));

    for storage in [0, 5, 25, 50, 85, 120] {
        let config = SimulationConfig {
            solar_capacity: 200.0,
            wind_capacity: 0.0, // No wind - matching user's config
        ...
        println!(
            "Storage: {:3} MWh -> Peak Gas: {:6.2} MW, Total Gas: {:6.0} MWh, Charge: {:4.0} MWh, Discharge: {:4.0} MWh",
            storage, peak_gas, total_gas, total_charge, total_discharge
        );
    }
}
